...
export class PSPlus {
    ...
    try {
      ...
      for (const title of gameTitles) {
        if (title && this.isValidGameTitle(title)) {
          const storeUrl = await this.getPSStoreUrl(title);
          games.push({
            title: title.trim(),
            description:
              type === "monthly"
  ...
  async getPSStoreUrl(gameTitle) {
    try {
      const cleanTitle = gameTitle
        .replace(/\s*\|\s*[^|]*$/g, "") // видаляє все після |
        .replace(/\s*\([^)]*\)/g, "") // видаляє дужки та їх вміст
        .replace(/\b(PS5|PS4|PSVR2?)\b/gi, "") // видаляє PS4/PS5 як окремі слова
        .replace(/\s+/g, " ")
        .trim();

      console.log(`🔍 Шукаю в PS Store: "${cleanTitle}"`);

      const searchQuery = encodeURIComponent(`"${cleanTitle}"`);
      const searchUrl = `https://store.playstation.com/uk-ua/search/${searchQuery}`;
      ...
      const response = await axios.get(searchUrl, {
        headers: {
          ...
          "Accept-Language": "uk-UA,uk;q=0.8,en;q=0.5,en-US;q=0.3",
        },
        timeout: 15000,
      });

      return this.extractUrlFromSearchPage(response.data, cleanTitle);
    } catch (error) {
      console.log(`❌ Помилка пошуку в PS Store: ${error.message}`);
      return null;
    }
  }

  extractUrlFromSearchPage(html, gameTitle) {
    try {
      const fallbackMatch = html.match(/href="(\/uk-ua\/product\/[^"]*)"/);
      if (fallbackMatch) {
        const productUrl = `https://store.playstation.com${fallbackMatch[1]}`;
        console.log(`✅ Знайдено посилання: ${productUrl}`);
        return productUrl;
      }

      console.log(`🔍 Посилання не знайдено, повертаю сторінку пошуку`);
      const searchQuery = encodeURIComponent(`"${gameTitle}"`);
      return `https://store.playstation.com/uk-ua/search/${searchQuery}`;
    } catch (error) {
