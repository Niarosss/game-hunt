...
export class PSPlus {
  ...
  async getMonthlyGames() {
    try {
      ...
      const articleUrl = await this.getLatestMonthlyArticleUrl();

      if (articleUrl) {
        const games = await this.parseArticleContent(articleUrl, "monthly");
  ...
  async getCatalogGames() {
    try {
      ...
      const articleUrl = await this.getLatestCatalogArticleUrl();

      if (articleUrl) {
        const games = await this.parseArticleContent(articleUrl, "catalog");
  ...
  async parseArticleContent(articleUrl, type) {
    try {
      console.log(`📖 Парсинг статті: ${articleUrl}`);
      ...
      const articleTitle = this.getArticleTitle(html);
      console.log("📰 Заголовок статті:", articleTitle);
  ...
  getArticleTitle(html) {
    ...
    return titleMatch ? this.cleanText(titleMatch[1]) : "PS Plus Games";
  }
  ...
  async getAllGames() {
    const [monthlyGames, catalogGames] = await Promise.all([
