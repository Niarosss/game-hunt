...
export class Steam {
  ...
  async getFreeGames() {
    console.info("🔄 Steam: запуск оновлення списку безкоштовних ігор...");
  ...
  async parseFreeGamesFromPage() {
    try {
      const url = `${this.baseURL}/search/?maxprice=free&category1=998&specials=1&ndl=1`;
  ...
  async getGameDetails(appId) {
    try {
      const { data } = await axios.get(
        `${this.baseURL}/api/appdetails?appids=${appId}&l=ukrainian`
      );
