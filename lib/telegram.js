...
export class TelegramBot {
  ...
  async sendPhoto(photoUrl, caption, parse_mode = "HTML") {
    if (!this.options.sendEnabled) {
      if (this.options.log) {
        console.log("ℹ️ Відправка повідомлень вимкнена (sendEnabled=false).");
      }
    ...
    try {
      ...
      console.error("❌ Помилка відправки фото:", error.response?.data);
      return false;
    }
  }
   ...
   */
  async sendGames({ source, games, article, imageUrl }) {
    const sourceConfig = {
    ...
    try {
      ...
      if (imageUrl) {
        return await this.sendPhoto(imageUrl, message);
      } else {
