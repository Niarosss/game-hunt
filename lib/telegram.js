...
export class TelegramBot {
  ...
  async sendPhoto(photoData, caption, parse_mode = "HTML") {
    if (!this.options.sendEnabled) {
      if (this.options.log) {
        console.log("ℹ️ Відправка повідомлень вимкнена (sendEnabled=false).");
      }
    ...
    // Блок для відправки зображення з буфера (для PS Plus)
    if (Buffer.isBuffer(photoData)) {
      const form = new FormData();
      form.append("chat_id", this.chatId);
      form.append("photo", photoData, "image.jpg");
      ...
      try {
        ...
        if (this.options.log)
          console.log("✅ Фото (з файлу) успішно надіслано.");
        return true;
      } catch (error) {
        console.error(
          "❌ Помилка відправки фото (з файлу):",
    ...
    // Блок для відправки зображення по URL (для Epic, Steam)
    const urls = Array.isArray(photoData) ? photoData : [photoData];
    if (urls.length === 0) return true;

    const isSingle = urls.length === 1;
    const urlEndpoint = isSingle ? endpoint : `${this.baseURL}/sendMediaGroup`;
  ...
  formatGamesMessage({ source, games, article }) {
    ...
    if (article?.startDate || article?.endDate) {
      ...
      if (article.startDate && article.endDate) {
        const from = new Date(article.startDate).toLocaleDateString(
          "uk-UA",
          format,
        );
        const to = new Date(article.endDate).toLocaleDateString(
          "uk-UA",
          format,
        );
        dateInfo = `з ${from} до ${to}`;
      } else if (article.startDate) {
        const from = new Date(article.startDate).toLocaleDateString(
          "uk-UA",
          format,
        );
      ...
      message += `\nℹ️ <i>Всі ігри доступні ${dateInfo}</i>\n\n`;
    }
  ...
  async sendGames({ source, games, article }) {
    ...
    try {
      ...
      if (imagesToSend.length > 0) {
        if (source === "psPlusMonthly" || source === "psPlusCatalog") {
          try {
            const imageUrl = imagesToSend[0];
            if (this.options.log)
              console.log(`📥 Завантажую зображення для ${source}...`);
            const response = await axios.get(imageUrl, {
              responseType: "arraybuffer",
            });
            const imageBuffer = Buffer.from(response.data);

            return await this.sendPhoto(imageBuffer, message);
          } catch (e) {
            console.error(
              `❌ Не вдалося завантажити зображення для ${source}: ${e.message}`,
            );
