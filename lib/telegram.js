...
export class TelegramBot {
  ...
  async sendNewEpicGames(games) {
    try {
      ...
      if (games.length === 0) {
        if (this.options.log) console.log("ℹ️ Немає нових Epic Games");
        return false;
      }
  ...
  async sendNewSteamGames(games) {
    try {
      ...
      if (games.length === 0) {
        if (this.options.log) console.log("ℹ️ Немає нових Steam ігор");
        return false;
      }
  ...
  async sendPSPlusMonthly(games, article) {
    try {
      ...
      if (games.length === 0) {
        if (this.options.log) console.log("ℹ️ Немає місячних ігор PS Plus");
        return false;
      }
  ...
  async sendPSPlusCatalog(games, article) {
    try {
      ...
      if (games.length === 0) {
        if (this.options.log) console.log("ℹ️ Немає ігор в каталозі PS Plus");
        return false;
      }
