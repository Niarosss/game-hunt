...
export class TelegramBot {
  ...
  formatPSPlusMonthlyMessage(games) {
    const currentDate = new Date().toLocaleDateString("uk-UA", {
      weekday: "long",
      year: "numeric",
      month: "long",
    ...
    message += `🔔 <i>Доступні для всіх підписників PS Plus</i>`;

    return message;
  }

  formatPSPlusCatalogMessage(games) {
    const currentDate = new Date().toLocaleDateString("uk-UA", {
      weekday: "long",
      year: "numeric",
      month: "long",
    ...
    message += `🔔 <i>Доступні для підписників PS Plus Extra та Premium</i>`;

    return message;
  }

  async sendPSPlusMonthly(games) {
    try {
      ...
      const message = this.formatPSPlusMonthlyMessage(games);
      return await this.sendMessage(message);
    } catch (error) {
  ...
  async sendPSPlusCatalog(games) {
    try {
      ...
      const message = this.formatPSPlusCatalogMessage(games);
      return await this.sendMessage(message);
    } catch (error) {
