  ...
  async sendGames({ source, games, article }) {
    ...
    try {
      ...
      if (source === "epic" || source === "steam") {
        ...
        // --- ПОЧАТОК ЗМІН ---
        // Вирішуємо URL, щоб отримати кінцеву адресу після можливих перенаправлень.
        // Це виправляє проблеми, коли Telegram не може обробити початковий URL з CDN.
        try {
          if (this.options.log)
            console.log(`🕵️  Вирішую URL зображення для ${source}...`);
          const response = await axios.head(article.imageUrl);
          const finalUrl = response.request.res.responseUrl || article.imageUrl;
          if (this.options.log) console.log(`✅ Кінцевий URL: ${finalUrl}`);
          imagesToSend.push(finalUrl);
        } catch (e) {
          console.error(
            `❌ Не вдалося вирішити URL зображення: ${article.imageUrl}`,
            e.message,
          );
          // Якщо не вдалося, спробуємо відправити оригінальний URL
